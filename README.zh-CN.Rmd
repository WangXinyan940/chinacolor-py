...
-   新增 `monochromatic_palette()`,`analogous_palette()`等10个自动生成调色板的函数；
...
![颜色图形](image/color/all_cirle.png){width="600"}

![带颜色分组编码的全部384种颜色](image/color/All_Colors_with%20_Group_ID.png){width="600"}

![按节气分组的颜色](image/color/colors_by_solar_term/2.雨水_Rain_Water.png){width="600"}

![调色板示例](image/palettes/palette_1.png){width="600"}

![调色板示例](image/palettes/palette_60.png){width="600"}
...
![](figures/list_colors.png){width="600"}
...
-   调色板类型只支持`sequential` ,`diverging`,`qualitative`这三种类型，定义为其他类型都将被默认为`qualitative`类型。
...
但对于Palette_C，我们认为其为发散型调色板，palette type 为`diverging`, 根据**更新的函数功能**，我们可以自定义其类型:
...
这里简单罗列几个函数生成调色板，并与[adobe CC](https://color.adobe.com/zh/create/color-wheel) 生成结果对比展示。
...
这里以 "#DB9C53"为基色，调色板颜色数量6,为基础，与adobe CC 的效果做个效果对比。
...
1.  `scale` 系列函数调色板输入进行了重要调整： 对于`*_d` 和`*_c`系列函数，上一版本支持内置调色板，更新后支持任何具有规定要求属性的调色板。

-   `*_d` 系列 支持`qualitative`类型调色板
-   `*_c` 系列 支持`seqential` 和`diverging`类型调色板

`ctc_palette`函数本次已进行更新，输出调色板有类型属性，该函数`custom`情形下的输出也可以用于`*_d` 和`*_c`系列函，这对上一版本是一个更新，后续scale系列函数中的`*_m`系列会逐渐取消。

`monochromatic_palette`等10个自动生成调色板的函数的输出，也可以直接用于`scale`系列函数；

甚至，任何一个自定义的颜色向量，可以人工赋值调色板类型属性，也是可以用于这个系列函数。

2.  `theme` 函数进行了更新，增加了一些方案，对`ink` 系列函数进行比较大的变动，上一版本的显示效果很差，本次更新，显示效果得到很大改善。
...
You can submit issues and report bugs via https://github.com/zhiming-chen/chinacolor/issues.

We also welcome contributions of color schemes and suggestions for improvements and optimizations.

Public Account, Zhihu: 空行马天君; QQ Email: [25172952@qq.com](mailto:25172952@qq.com)
