#' @title Generate an Antipodal (Split-Complementary) Color Palette
#' @description This function generates a color palette based on an antipodal color scheme,
#'   emphasizing opposing (i.e., ~180-degree separated) hues on the color wheel.
#'   Starting from a base color, it selects four primary hues at angles of 0°, +45°, +180°,
#'   and +225° relative to the base hue. This creates a split-complementary structure
#'   with one base color, its complement (antipodal), and two adjacent colors near the complement.
#'   For each of these four primary colors, monochromatic variants are generated to expand
#'   the palette to the desired total number of colors (`n`). Optionally, the resulting colors
#'   can be mapped to the closest traditional Chinese colors from the \code{chinacolor} dataset.
#'
...
#' @examples
#' \dontrun{
#' # Generate a 10-color antipodal palette from a specific base color
#' antipodal_palette(base_color = "#779649", n = 10, ctc_colors = TRUE, show_pal = TRUE)
#'
#' # Generate an 8-color palette using a random base color
#' antipodal_palette(n = 8, ctc_colors = TRUE, show_pal = TRUE)
#' }
#'
#' @export
antipodal_palette <- function(base_color = NULL,
                                                n = 8,
                                                ctc_colors = TRUE,
                                                show_pal = TRUE) {
    ...
    contrast_angle <- 45
    hues <-  (base_hue + contrast_angle * c(0,1,4,5)) %% 360
